...
bool UcppdataBaseManager::CreateNewDatabase()
{
    FString DatabasePath = FPaths::ProjectContentDir() + "Database/temp2025items_unreal.db";

    bool bIsDatabaseOpen = DBConnection.Open(*DatabasePath, nullptr, nullptr);
    if (!bIsDatabaseOpen)
    {
        UE_LOG(LogTemp, Error, TEXT("[DB_ERROR] Failed to create new Unreal DB at: %s"), *DatabasePath);
        return false;
    }

    UE_LOG(LogTemp, Log, TEXT("[DB_DEBUG] Successfully created new Unreal DB at: %s"), *DatabasePath);
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("[DB_ERROR] Failed to create Items table in Unreal DB."));
        return false;
    }

    UE_LOG(LogTemp, Log, TEXT("[DB_DEBUG] Items table created successfully in Unreal DB."));
    return true;
}

bool UcppdataBaseManager::InsertTestData()
{
    FString InsertQuery = TEXT(R"(
        INSERT INTO Items (Name, Type, Value, Weight, ImagePath, ModelID) 
        VALUES 
        ('TestSword', 'Weapon', 100, 5.5, 'path/to/image1.png', 'model_001'),
        ('TestShield', 'Armor', 150, 10.0, 'path/to/image2.png', 'model_002');
    )");
...
bool UcppdataBaseManager::OpenUnrealDatabase()
{
    FString DatabasePath = FPaths::ProjectContentDir() + "Database/temp2025items_unreal.db";
    ...
    if (!bIsDatabaseOpen)
    {
        UE_LOG(LogTemp, Error, TEXT("[DB_ERROR] Failed to open Unreal DB at: %s"), *DatabasePath);
        return false;
    }

    UE_LOG(LogTemp, Log, TEXT("[DB_DEBUG] Opened Unreal-generated DB at: %s"), *DatabasePath);
    return true;
}
